...
系统资源边界由\texttt{scaffold\_chunks.yaml}控制，核心配置项如表\ref{tab:config_behavior}所示，并在\texttt{trainer.cpp}中映射到锚点、chunk、优化、相机和关键帧五组配置。关键配置不是简单超参数，而是直接改变数据流粒度：offset数量决定每个锚点生成的子高斯数，特征维度决定MLP输入和压缩容量，chunk边长决定I/O空间粒度，GPU锚点上限决定resident set规模，损失权重决定反向传播对颜色、深度和形状的约束比例。
...
表\ref{tab:fileorg}给出本章叙事与真实源文件的对应关系。
